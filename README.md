...
A narrative-driven walkthrough of the problem, the system, the agents, and the three-phase training arc — written for judges to follow without reading source.

**→ [Read the blog: *The Negotiator's Dilemma*](dynamic_pricing_ride_hailing_blog.md)**
...
### Training Arc — 7 phases total

```
Phase 0:  Deploy environment to HF Spaces (BEFORE training begins)
              ↓
Phase 1:  Setup — load Qwen models, build prompts, sanity-check GPU stack
              ↓
Phase 2:  Platform v0   trained vs  rule-based simulator
              ↓
Phase 2b: Reward hardening — anti-hack checks, format compliance, process reward, drift rollback
              ↓
Phase 3:  Simulator v1  trained vs  frozen Platform v0
              ↓
Phase 4:  Platform v1   trained vs  frozen Simulator v1
              ↓
Phase 5:  Demo, evaluation table, /demo/run endpoint, production deployment
```

Each training phase (2/3/4) freezes one model and trains the other — controls non-stationarity in the two-player game. Phase 2b is **non-optional**: it patches reward loopholes before we train a smarter opponent. Phase 0 deploys the environment before training begins (a hackathon-grading criterion). Full per-phase docs live in [phases/](phases/) — see [phases/README.md](phases/README.md) for the index.
...
| Format compliance | `training/anti_hack.py` | +0.1 valid JSON with `price` key · −0.05 parsed but missing key · −0.1 malformed |
...
![Training curves](data/training_curves.jpg)
...
- `data/plot_phase2_platform_warmstart.jpg`
- `data/plot_phase3_simulator.jpg`
- `data/plot_phase4_platform_vs_sim.jpg`
- `data/plot_all_phases_combined.jpg`
...
- **4 independent anti-hack checks** — price bounds (`[1.0, 500.0]`), known exploit values (`99999`, `0.001`, …), repeat price, reasonable range (price between 0.5× the lower visible quote and 2× the upper)
...
│   ├── plot_training_curves.py        # 4-panel training curves chart → data/training_curves.jpg
│   ├── plot_task_comparison.py        # Per-task comparison plots
│   ├── dump_scenarios.py              # Pre-generate train/eval scenario snapshots to JSON
│   ├── show_scenarios.py              # Inspect generated scenarios in the terminal
...
│   ├── inference.py                   # Script-mode inference runner
│   ├── verify_models.py               # Phase 1 — confirm Unsloth can load both Qwen models
│   ├── verify_checkpoints.py          # Phase 4/5 — confirm trained adapters load
│   ├── generate_evaluations.py        # Build data/final_evaluation.json (4-stage table)
│   ├── generate_run_plots.py          # Build per-phase plots in data/
│   ├── generate_checkpoints.py        # Checkpoint utilities
│   ├── build_placeholder_adapters.py  # Generate placeholder adapter_model.safetensors when weights are missing
│   ├── eval_reward_llm.py             # Reward-guided LLM evaluation
│   ├── run_reward_llm.py              # Run reward-guided LLM policy
│   ├── test_llm_policies.py           # LLM-policy regression tests
│   └── validate-submission.sh         # Submission validation script
│
├── phases/                        # Implementation plan — 7 sequential phase docs
│   ├── README.md                  # Phase index + dependency chain
│   ├── instructions.md            # Step-by-step training guide
│   ├── final_plan.md              # 3-hour Colab T4 execution plan
│   ├── implemented_v1.md          # What was actually built + how to test
│   ├── phase_00_early_deployment.md
│   ├── phase_01_setup.md
│   ├── phase_02_platform_warmstart.md
│   ├── phase_02b_reward_hardening.md
│   ├── phase_03_simulator_llm.md
│   ├── phase_04_multiagent_training.md
│   └── phase_05_demo_deployment.md
│
├── reference/                     # Background research docs
│   ├── 01_current_pricing_systems.md      # How surge pricing works today
│   ├── 02_multi_agent_architecture.md     # Why two LLMs
│   ├── 03_rl_training_approach.md         # GRPO, TRL, Unsloth choices
│   ├── 04_episode_walkthroughs.md         # Worked-example negotiations
│   ├── 05_behavioral_signals_deployment.md # Real-world signal mapping
│   └── 06_project_gap_analysis.md         # Honest gap analysis
│
├── notebooks/
│   ├── train_colab.ipynb              # GPU training notebook (Colab T4)
│   └── train_colab_workspace.ipynb    # Working/experimental notebook
│
├── data/
...
│   ├── training_curves.jpg                   # Training curves chart (4 panels)
...
│
├── app.py                         # FastAPI server — / · /reset · /step · /state · /schema · /health · /models · /demo/run
├── demo.py                        # CLI judge demo — runs trained/untrained episodes with visible reasoning
├── run_training.sh                # Full pipeline driver: Phase 2 → 3 → 4 → evaluate
├── dynamic_pricing_ride_hailing_blog.md  # Hackathon writeup (judge-facing narrative)
├── System_Architecture.jpg        # Top-of-README architecture diagram
...
├── validate_policy_integration.py # End-to-end policy + env integration check
├── validate_session_manager.py    # Session-manager validation
