...
### 4-Stage Evaluation Table

Completion rate across all task difficulties:

| Stage | Description | Easy | Medium | Hard |
|---|---|---|---|---|
| **A** | Untrained platform vs rule-based | ~25% | ~15% | ~8% |
| **B** | Platform v0 vs rule-based (Phase 2) | ~60% | ~40% | ~20% |
| **C** | Platform v0 vs Simulator v1 (Phase 3 disruption) | ~40% | ~25% | ~12% |
| **D** | Platform v1 vs Simulator v1 (Phase 4 recovery) | ~70% | ~50% | ~28% |

> Row C dropping below Row B is the **disruption moment** — proof the simulator LLM learned genuine strategic bluffing.
> Row D recovering above Row B is the **adaptation story** — proof the platform RL loop worked.

_After training, the actual results from `data/final_evaluation.json` replace the estimates above._

### Training Curves

![Training curves](data/training_curves.png)

_Generated by `python scripts/plot_training_curves.py` after training. Shows: platform reward curve, completion/cancel/timeout rates, simulator reward, bluff rate and collapse rate._

### Training Logs

Full step-level metrics saved to:
- `data/training_metrics_platform_easy.json` — 8 columns every 50 steps (reward, completion, cancel, timeout, grpo_loss, hacks)
- `data/training_metrics_simulator_easy.json` — bluff_rate, collapse_rate, grpo_loss every 50 steps

Sample log output during Phase 4 training:
```
[STEP   50] avg=2.341  win=1.823  done=62%  cancel=18%  timeout=20%  loss=0.0234  hacks=0  (12.3s)
[STEP  100] avg=3.102  win=2.614  done=68%  cancel=15%  timeout=17%  loss=0.0198  hacks=0  (11.8s)
[STEP  200] avg=4.218  win=3.891  done=72%  cancel=12%  timeout=16%  loss=0.0156  hacks=0  (12.1s)
```
...
- **Drift detection + auto-rollback** — rolls back to last stable checkpoint if reward drops >30%
